	...
	for k,v in pairs(level.entities) do
		...
		if shown then
			...
			and mouseon(offsetx+(v.x-myroomx*40)*16, offsety+(v.y-myroomy*30)*16, 16, 16) then
				...
				elseif v.t == 9 then
					-- Trinket
					menu = {"#" .. toolnames[4], L.DELETE, L.MOVEENTITY, (level.count.trinkets >= level.limit.trinkets and "#" or "") .. L.COPY, L.PROPERTIES}
				elseif v.t == 10 then
					-- Checkpoint
				...
				elseif v.t == 15 then
					-- Rescuable crewmate
					menu = {"#" .. toolnames[16], L.DELETE, L.CHANGECOLOR, L.MOVEENTITY, (level.count.crewmates >= level.limit.crewmates and "#" or "") .. L.COPY, L.PROPERTIES}
				elseif v.t == 16 then
					-- Start point
