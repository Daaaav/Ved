...
function Font_getWrap9(thefont, thetext, thelimit)
	-- Basically, be LÖVE 0.9's Font:getWrap(), if it also provided the table of wrapped lines like it did in 0.10

	-- Split the string into words (strings separated by spaces). Ignore multiple spaces between words. Strip leading and trailing whitespace
	local thewords = {}
	for word in thetext:gmatch("%S+") do
		table.insert(thewords, word)
	end

	-- Now put it back together into multiple lines
	local theresult = {}
	local theline = {}
	local currentlinewidth = 0
	for n, word in pairs(thewords) do
		local thewidth = thefont:getWidth(word)

		if currentlinewidth + thewidth > thelimit then
			-- Make a new line
			if #theline == 0 then
				-- Too big for one line already?
				table.insert(theresult, word .. " ")
				currentlinewidth = 0
				theline = {}
			else
				table.insert(theresult, table.concat(theline, " ") .. " ")
				currentlinewidth = 0
				theline = {}

				-- Don't forget to insert the word that would've overflowed the line
				-- Also, this code doesn't check if the word is still too big for one line anyway, but 0.9's getWrap didn't either
				table.insert(theline, word)
				currentlinewidth = currentlinewidth + thewidth + thefont:getWidth(" ")
			end
		else
			table.insert(theline, word)
			currentlinewidth = currentlinewidth + thewidth + thefont:getWidth(" ")
	...
	if #theline > 0 then
		table.insert(theresult, table.concat(theline, " ") .. " ")
		-- Yes, we always add the last space even when there is none at the end.
		-- Don't think this will cause any issues for the cases where is no space at the end
	end

	local linewidths = {}
	for _, line in pairs(theresult) do
		table.insert(linewidths, thefont:getWidth(line))
	end

	return math.max(unpack(linewidths)), theresult
end
