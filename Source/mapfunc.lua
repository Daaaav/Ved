...
function map_doroom(x, y)
	...
	if love.graphics.isSupported("canvas") then
		...
		if s.mapstyle == "minimap" then
			displayminimaproom(0, 0, level:get_tiles(x, y), level:get_roommetadata(x, y))
		elseif s.mapstyle == "vtools" then
			displayvtoolsroom(0, 0, level:get_tiles(x, y), level:get_roommetadata(x, y))
		else
			displayroom(0, 0, level:get_tiles(x, y), level:get_roommetadata(x, y), 0.5)
		end
