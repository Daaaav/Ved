...
typedef struct _ArabicLigature
{
    uint32_t source[2];
    uint32_t target;
    bool mandatory;
...
const uint32_t* bidi_transform_utf32(const bool rtl, const uint32_t* text, VisualLayoutGlyph* layout, size_t layout_n)
{
    ...
    // Even though the input is already UTF-32, we still want to copy
    uint32_t codepoint;
    size_t text_i = 0;
    ...
    for (SBUInteger i = 0; i < n_runs; i++)
    {
        ...
        {
            // Time for reshaping!
            typedef enum _arabic_form { NONE, ISOLATED, INITIAL, MEDIAL, FINAL } arabic_form;
        ...
        for (size_t c = 0; c < runs[i].length; c++)
        {
            ...
            int out_room_left = sizeof(utf32_out)/sizeof(uint32_t) - 1 - utf32_out_cur;
            if (out_room_left <= 0)
            {
                goto no_more_runs;
            }
