...
return function()
	...
	for script_i = #level.scriptnames, 1, -1 do
		...
		or scriptdisplay_used and usedscripts[level.scriptnames[script_i]]
		or scriptdisplay_unused and not usedscripts[level.scriptnames[script_i]]
		then
			j = j + 1
			...
			elseif y >= -16 and y <= love.graphics.getHeight() then
				local used = usedscripts[level.scriptnames[script_i]]
				hoverrectangle(128,128,128, used and 128 or 64, 8, y, screenoffset+640-8-24 -36, 16)
				font_level:printf(level.scriptnames[script_i], 8, y+4, screenoffset+640-8-36, "center")
				if script_i == #level.scriptnames then
			...
			elseif script_i ~= #level.scriptnames
			and mouseon(8+screenoffset+640-8-24 -36 +4, scriptlistscroll+8+(24*j), 16, 16)
			and love.mouse.isDown("l") then
				movescriptdown(script_i)
				mousepressed = true
				local nextscriptvisible = (scriptdisplay_used and scriptdisplay_unused)
					or (scriptdisplay_used and usedscripts[level.scriptnames[script_i]])
					or (scriptdisplay_unused and not usedscripts[level.scriptnames[script_i]])
			...
			elseif script_i ~= 1
			and mouseon(8+screenoffset+640-8-24 -36 +4 +16 +4, scriptlistscroll+8+(24*j), 16, 16)
			and love.mouse.isDown("l") then
				...
				elseif scriptlistscroll+8+24*(j+1) +16 > love.graphics.getHeight() then
					newscroll = scriptlistscroll - 24
					local height = -(((#level.scriptnames)*24-8)-(love.graphics.getHeight()-16))
