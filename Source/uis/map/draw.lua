...
return function()
	...
	local startx, starty
	if level.entities[level.count.startpoint] ~= nil then
		startx = math.floor(level.entities[level.count.startpoint].x/40)
		starty = math.floor(level.entities[level.count.startpoint].y/30)
		...
		for mrx = 0, level.metadata.mapwidth-1 do
			...
			or (selectedtool == 17 and mrx == startx and mry == starty) then
				...
				elseif selectedtool == 16 then
					...
					amount = 1
					spritefunc = function() return 3*level.entities[level.count.startpoint].p1 end
					width = 10
					widthb = 12
					extray = -2
					scalesubtract = 21
