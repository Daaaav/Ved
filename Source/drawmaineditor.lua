function drawmaineditor()
	...
	if tilespicker then
		...
		if s.adjacentroomlines then
			...
			if levelmetadata_get(roomx, roomy).warpdir == 2 or levelmetadata_get(roomx, roomy).warpdir == 3 then
			...
			-- Room left
			if levelmetadata_get(roomx, roomy).warpdir == 1 or levelmetadata_get(roomx, roomy).warpdir == 3 then
				-- Use this room because it warps.
				roomleft = roomx
				roomleftW = true
			else
			...
			for t = 0, 39 do
				...
				elseif not roomupW and ( (levelmetadata_get(roomx, roomup).warpdir == 2) or (levelmetadata_get(roomx, roomup).warpdir == 3) ) then
			...
			for t = 0, 29 do
				...
				elseif not roomrightW and ( (levelmetadata_get(roomright, roomy).warpdir == 1) or (levelmetadata_get(roomright, roomy).warpdir == 3) ) then
			...
			for t = 0, 39 do
				...
				elseif not roomdownW and ( (levelmetadata_get(roomx, roomdown).warpdir == 2) or (levelmetadata_get(roomx, roomdown).warpdir == 3) ) then
		...
		local hasroomname = levelmetadata_get(roomx, roomy).roomname ~= ""
	...
	_= not voided_metadata and rbutton(not editingroomname and {langkeys(L.WARPDIR, {warpdirs[levelmetadata_get(roomx, roomy).warpdir]}), "W"} or langkeys(L.WARPDIR, {warpdirs[levelmetadata(roomx, roomy).warpdir]}), 3+additionalbutton_np, additionalbutton_yoffset, true, additionalbutton_spacing)
	...
	if selectedtool == 4 or selectedtool == 8 or selectedtool == 9 or selectedtool == 16 then
		...
		if not mousepressed and nodialog and love.mouse.isDown("l") then
			...
			elseif onrbutton(-2, 164+4, true) and selectedtool == 9 then
				...
				switchenemies()
				mousepressed = true
			elseif mouseon(love.graphics.getWidth()-(128-8)+(6*8) + (64 - font8:getWidth(levelmetadata_get(roomx, roomy).platv))/2 - 4, love.graphics.getHeight()-(24*(-2+1))-(160), font8:getWidth(levelmetadata_get(roomx, roomy).platv) + 8, 16) and selectedtool == 8 then
				-- Platform speed
				dialog.create(
					L.PLATVCHANGE_MSG,
					DBS.OKCANCEL,
